...
class Patient:
    def __init__(self, patient_id, doctor_id, name, birth_year, gender, phone, time, medical_record):
    ...
    def add_new_patient(self):
        clinic_storage.modify_system_log(f"新患者 - | {self.time:^20} | 患者 ID: {self.patient_id:^5} | 名字: {self.name:^15} | 电话: {self.phone} | 性别: {self.gender:^2} | 出生年: {self.birth_year} |")
        doctor_name = data_storage.doctor_storage.identify_doctor(self.doctor_id)
        data_storage.patient_storage.store_new_patient(self.patient_id, [doctor_name, self.name, self.gender, self.birth_year, self.phone, self.time, self.medical_record])
...
class Patient_Symptom:
    def __init__(self, patient_id, symptom_id, doctor_id, complaint,\
                  diagnosis, treatment_course, note, diagnosis_time):
